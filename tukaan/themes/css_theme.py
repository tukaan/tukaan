...
class TclThemeGenerator:
    ...
    def generate(self) -> str:
        return 'puts "theme_generate"'

    def generate_theme_use(self) -> str:
        return 'puts "theme_use"'


class CssTheme(Theme):
    def __init__(self, file_path: Path) -> None:
        css_theme_data = CssThemeParser.load(file_path)
        tcl_theme_generator = TclThemeGenerator(css_theme_data)

        Tcl.eval(None, tcl_theme_generator.generate())

        self._theme_use_script = tcl_theme_generator.generate_theme_use()

    def _use(self) -> None:
        Tcl.eval(None, self._theme_use_script)
